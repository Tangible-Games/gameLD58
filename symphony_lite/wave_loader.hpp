...
bool WaveFile::Load(const std::string& file_path, WaveFile::Mode mode) {
  ...
  if (!file.is_open()) {
    std::cerr << "[WaveFile] Can't open file, file_path: " << file_path
              << std::endl;
    return false;
  }
  ...
  if (!riff.TestChunk("RIFF")) {
    std::cerr << "[WaveFile] Not a RIFF file, file_path: " << file_path
              << std::endl;
    return false;
  }
  ...
  if (bytes_to_scan < 4) {
    std::cerr << "[WaveFile] File too small, file_path: " << file_path
              << std::endl;
    return false;
  }
  ...
  if (format_id[0] != 'W' || format_id[1] != 'A' || format_id[2] != 'V' ||
      format_id[3] != 'E') {
    std::cerr << "[WaveFile] Not a WAVE file, file_path: " << file_path
              << std::endl;
    return false;
  }
  ...
  while (bytes_to_scan >= sizeof(RiffChunkHeader)) {
    ...
    if (bytes_to_scan < chunk.GetSize()) {
      std::cerr << "[WaveFile] File is misconfigured, file_path: " << file_path
                << std::endl;
      return false;
    }
    ...
    if (chunk.TestChunk("fmt ")) {
      ...
      if (format_common_.GetFormatCategory() != kWaveFormatPCM) {
        std::cerr << "[WaveFile] Format is not supported, file_path: "
                  << file_path << std::endl;
        return false;
      }
      ...
      if (!format_read) {
        std::cerr
            << "[WaveFile] Data chunk should follow format chunk, file_path: "
            << file_path << std::endl;
        return false;
      }
  ...
  if (!format_read || !wave_data_read) {
    std::cerr << "[WaveFile] File doesn't contain needed chunks, file_path: "
              << file_path << std::endl;
    return false;
  }
  ...
  if (mode == kModeLoadInMemory) {
    ...
    file.read(&wave_data_[0], wave_data_size_);
  }
...
void WaveFile::ReadSamples(size_t first_sample_offset_in_bytes,
  ...
  if (wave_data_.empty()) {
    ...
    file.seekg(wave_data_offset_ + first_sample_offset_in_bytes, std::ios::beg);
    file.read((char*)buffer_out, size_in_bytes);
  }
}
