...
TEST(FormattedText, SingleLine) {
  auto formatted_text = LoadFormattedText(
      "<style font=\"system_24.fnt\">Fps:</> <style "
      "font=\"system_50.fnt\"><sub variable=\"$fps_count\"></>",
      Style(), HorizontalAlignment::kLeft, {{"fps_count", "60"}});
...
TEST(FormattedText, OutputsLessThanSign) {
  auto formatted_text =
      LoadFormattedText("<<<style font=\"system_24.fnt\">Text</>", Style(),
...
TEST(FormattedText, SpecifyStyle) {
  auto formatted_text = LoadFormattedText(
      ...
      "align=\"right\">Text</></></>",
      Style(), HorizontalAlignment::kLeft, {});
  ASSERT_TRUE(formatted_text.has_value());
  ASSERT_EQ((int)formatted_text->paragraphs.size(), 1);
  ...
  ASSERT_EQ(formatted_text->paragraphs[0].style_runs[0].style.color,
            0xFFFF0000);
  ASSERT_EQ(formatted_text->paragraphs[0].align, HorizontalAlignment::kRight);
}
...
TEST(FormattedText, AlignsParagraphs) {
  ...
  ASSERT_EQ(formatted_text->paragraphs[0].align, HorizontalAlignment::kLeft);
  ASSERT_EQ(formatted_text->paragraphs[0].style_runs[0].text, "Left align");
  ASSERT_EQ(formatted_text->paragraphs[1].align, HorizontalAlignment::kRight);
  ASSERT_EQ(formatted_text->paragraphs[1].style_runs[0].text, "Right align");
  ASSERT_EQ(formatted_text->paragraphs[2].align, HorizontalAlignment::kCenter);
  ASSERT_EQ(formatted_text->paragraphs[2].style_runs[0].text, "Center align");
  ASSERT_EQ(formatted_text->paragraphs[3].align, HorizontalAlignment::kLeft);
