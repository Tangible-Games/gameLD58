...
namespace gameLD58 {
class DefeatScreen : public Keyboard::Callback {
 public:
  class Callback {
   public:
    virtual void ContinueFromDefeatScreen() = 0;
  };

  DefeatScreen(std::shared_ptr<SDL_Renderer> renderer,
...
void DefeatScreen::Load() {
  image_.reset(IMG_LoadTexture(renderer_.get(), "assets/failure.png"),
               &SDL_DestroyTexture);
}

void DefeatScreen::Update(float /*dt*/) {}
...
void DefeatScreen::OnKeyDown(Keyboard::Key /*key*/) {}

void DefeatScreen::OnKeyUp(Keyboard::Key key) {
  if (key == Keyboard::Key::kX) {
    audio_->Play(all_audio_->audio[Sound::kButtonClick],
                 Symphony::Audio::PlayTimes(1), Symphony::Audio::kNoFade);

    if (callback_) {
      callback_->ContinueFromDefeatScreen();
    }
  }
}

void DefeatScreen::RegisterCallback(Callback* callback) {
  callback_ = callback;
}
}  // namespace gameLD58
